...
def transform_text(messages, word_dictionary):
    ...
    """
    # *** START CODE HERE ***
    memory = np.zeros([len(messages),len(word_dictionary)])
    for i, message in enumerate(messages):
        for word in get_words(message):
            if word in word_dictionary:
                memory[i, word_dictionary[word]] = 1
    return memory
...
def fit_naive_bayes_model(matrix, labels):
    ...
    # *** START CODE HERE ***
    m, n = matrix.shape
    # phi_1 = (labels.dot(matrix))/(sum(labels == 1))
    # phi_0 = ((1-labels).dot(matrix))/(sum(labels == 0))
    phi_1 = (labels.dot(matrix)+1)/(sum(labels == 1)+2)
    phi_0 = ((1-labels).dot(matrix)+1)/(sum(labels == 0)+2)
    # phi_1 = (labels.dot(matrix)+1)/sum(labels.dot(matrix)+1)
    # phi_0 = ((1-labels).dot(matrix)+1)/sum((1-labels).dot(matrix)+1)
    phi = sum(labels == 1)/m
    ph = np.array([phi_0, phi_1])
    return ph, phi
...
def predict_from_naive_bayes_model(model, matrix):
    ...
    ph, phi = model
    ll = matrix.dot(np.nan_to_num(np.log(ph)).T)+(1-matrix).dot(np.log(1-ph).T)+np.log([1-phi,phi])[None,:]
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    dic = list(dictionary)
    words = [dic[ind] for ind in indices]
    words.reverse()
    return words
    # *** END CODE HERE ***
...
def compute_best_svm_radius(train_matrix, train_labels, val_matrix, val_labels, radius_to_consider):
    ...
    for radius in radius_to_consider:
        ...
        acc.append(np.mean(pred == val_labels))
    op_radius = radius_to_consider[np.argmax(acc)]
    return op_radius
    # *** END CODE HERE ***
